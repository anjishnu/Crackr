...
def extract(text):
	...
	limit = 5 #print sortedKeywords[0:(totalKeywords/3)]
    #Limiter limits the max number
	lols = defaultdict(list)
	for tup in sortedKeywords:
		l = len(tup[0].split())
		if (l<=3) and len(lols[str(l-1)])<limit:
		    lols[str(l-1)] += [tup[0]]
	return dict(lols)